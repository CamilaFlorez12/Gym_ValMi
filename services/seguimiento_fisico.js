...
class Seguimiento_fisico {
    ...
    async registrarAvance({ peso, grasa, medidas, fotos = [], comentario }) {
        ...
        // convierto string a number si es posible
        if (typeof medidas === "string" && !isNaN(Number(medidas))) {
            medidas = Number(medidas);
        }

        // validación
        if (
            (typeof medidas !== "object" && typeof medidas !== "number") ||
            (typeof medidas === "object" && (Array.isArray(medidas) || Object.values(medidas).some(v => typeof v !== "number")))
        ) {
            throw new Error("Las medidas deben ser un número o un objeto con valores numéricos.");
        }
